...
{    
    ...
    public function  __construct() {
        $where['code']=['eq','alipay'];
        $where['type']=['eq','payment'];
        $paymentPlugin = \think\Db::name('plugin')->where($where)->find(); // 找到支付插件的配置
    ...
    function get_code($order, $config_value){
            ...
            $parameter = array(
                        ...
                        "notify_url"	=> SITE_URL.url('Payment/notifyUrl',array('pay_code'=>'alipay')) , //服务器异步通知页面路径 //必填，不能修改
                        "return_url"	=> SITE_URL.url('Payment/returnUrl',array('pay_code'=>'alipay')),  //页面跳转同步通知页面路径
                        "out_trade_no"	=> $order['order_sn'], //商户订单号                        
                        "subject"	=> '订单支付', //订单名称 可以中文
    ...
    function response(){
        ...
        /*write_text($verify_result);
        write_text('/n/r-------------------/n/r');
        write_text($_POST);
        write_text('/n/r-------------------/n/r');
        write_text(date('Y-m-d H:i:s',time()));*/
