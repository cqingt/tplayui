        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <section class="panel panel-padding">
                <form id="form1" class="layui-form layui-form-pane" action="<?php echo url(''); ?>">
                    <?php if(is_array($plugin['config']) || $plugin['config'] instanceof \think\Collection || $plugin['config'] instanceof \think\Paginator): $i = 0; $__LIST__ = $plugin['config'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <div class="layui-form-item">
                        <label class="layui-form-label"> <?php echo $vo['label']; ?> </label>
                        <?php if($vo['type'] == 'select'): ?>
                        <div class="layui-input-block">
                            <select name="config[<?php echo $vo['name']; ?>]">
                                <?php if(is_array($vo['option']) || $vo['option'] instanceof \think\Collection || $vo['option'] instanceof \think\Paginator): $o = 0; $__LIST__ = $vo['option'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($o % 2 );++$o;?>
                                <option <?php if($config_value[$vo['name']] == $o): ?>selected<?php endif; ?>  value="<?php echo $o; ?>"><?php echo $v; ?></option>
                                <?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                        <?php elseif($vo['type'] == 'textarea'): ?>
                        <div class="layui-input-block">
                            <textarea name="config[<?php echo $vo['name']; ?>]" class="layui-textarea" placeholder="请输入<?php echo $vo['label']; ?>"><?php echo (isset($config_value[$vo['name']]) && ($config_value[$vo['name']] !== '')?$config_value[$vo['name']]:''); ?></textarea>
                        </div>
                        <?php else: ?>
                            <div class="layui-input-block">
                            <input type="password" value="<?php echo $config_value[$vo['name']]; ?>" name="config[<?php echo $vo['name']; ?>]"  placeholder="请输入<?php echo $vo['label']; ?>" class="layui-input "/>
                            </div>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
